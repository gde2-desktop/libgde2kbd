...
{
	Gde2kbdKeyboardDrawingGroupLevel *pgl = groupLevels;
	gint cntr, g;
...
static void
bad_keycode (Gde2kbdKeyboardDrawing * drawing, guint keycode)
{
	g_warning
...
{
	GtkWidget *window;
	GtkWidget *gde2kbd_keyboard_drawing;
	GdkScreen *screen;
	gint monitor;
	GdkRectangle rect;
	...
	Gde2kbdKeyboardDrawingGroupLevel groupLevels[4] =
	    { {0, 0}, {1, 0}, {0, 1}, {1, 1} };
	Gde2kbdKeyboardDrawingGroupLevel *pgroupLevels[4] =
	...
	gde2kbd_keyboard_drawing = gde2kbd_keyboard_drawing_new ();
	gtk_widget_show (gde2kbd_keyboard_drawing);
	gtk_container_add (GTK_CONTAINER (window), gde2kbd_keyboard_drawing);
	...
	if (track_modifiers)
		gde2kbd_keyboard_drawing_set_track_modifiers
		    (GDE2KBD_KEYBOARD_DRAWING (gde2kbd_keyboard_drawing), TRUE);
	if (track_config)
		gde2kbd_keyboard_drawing_set_track_config
		    (GDE2KBD_KEYBOARD_DRAWING (gde2kbd_keyboard_drawing), TRUE);
	g_signal_connect (G_OBJECT (gde2kbd_keyboard_drawing), "bad-keycode",
	...
	if (symbols || geometry || keycodes) {
		...
		if (symbols)
			...
			names.symbols = (gchar *)
			    gde2kbd_keyboard_drawing_get_symbols
			    (GDE2KBD_KEYBOARD_DRAWING
			     (gde2kbd_keyboard_drawing));
		...
		if (keycodes)
			...
			names.keycodes = (gchar *)
			    gde2kbd_keyboard_drawing_get_keycodes
			    (GDE2KBD_KEYBOARD_DRAWING
			     (gde2kbd_keyboard_drawing));
		...
		if (geometry)
			...
			names.geometry = (gchar *)
			    gde2kbd_keyboard_drawing_get_geometry
			    (GDE2KBD_KEYBOARD_DRAWING
			     (gde2kbd_keyboard_drawing));

		success =
		    gde2kbd_keyboard_drawing_set_keyboard
		    (GDE2KBD_KEYBOARD_DRAWING (gde2kbd_keyboard_drawing),
		     &names);
		if (!success) {
			g_printerr
	...
	gtk_widget_grab_focus (gde2kbd_keyboard_drawing);
