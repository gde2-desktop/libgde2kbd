...
typedef struct _gki_globals {
	...
	GSList *widget_instances;	/* list of Gde2kbdStatus */
...
#define ForAllIndicators() \
	{ \
		...
		for (cur = globals.widget_instances; cur != NULL; cur = cur->next) { \
			Gde2kbdStatus * gki = (Gde2kbdStatus*)cur->data;
#define NextIndicator() \
		} \
	}

G_DEFINE_TYPE (Gde2kbdStatus, gde2kbd_status, GTK_TYPE_STATUS_ICON)
static void
gde2kbd_status_global_init (void);
static void
gde2kbd_status_global_term (void);
static GdkPixbuf *
gde2kbd_status_prepare_drawing (Gde2kbdStatus * gki, int group);
static void
gde2kbd_status_set_current_page_for_group (Gde2kbdStatus * gki, int group);
static void
gde2kbd_status_set_current_page (Gde2kbdStatus * gki);
static void
gde2kbd_status_global_cleanup (Gde2kbdStatus * gki);
static void
gde2kbd_status_global_fill (Gde2kbdStatus * gki);
static void
gde2kbd_status_set_tooltips (Gde2kbdStatus * gki, const char *str);
...
gde2kbd_status_global_fill (Gde2kbdStatus * gki)
{
	...
	for (grp = 0; grp < total_groups; grp++) {
		GdkPixbuf *page = gde2kbd_status_prepare_drawing (gki, grp);
		globals.icons = g_slist_append (globals.icons, page);
	}
}

static void
gde2kbd_status_activate (Gde2kbdStatus * gki)
{
...
/* hackish xref */
extern gchar *gde2kbd_indicator_extract_layout_name (int group,
						  XklEngine * engine,
...
extern gchar *gde2kbd_indicator_create_label_title (int group,
						 GHashTable **
						 ln2cnt_map,
						 gchar * layout_name);

static void
gde2kbd_status_render_cairo (cairo_t * cr, int group)
{
	double r, g, b;
	...
	layout_name = gde2kbd_indicator_extract_layout_name (group,
							  globals.engine,
							  &globals.kbd_cfg,
							  globals.short_group_names,
							  globals.full_group_names);
	lbl_title =
	    gde2kbd_indicator_create_label_title (group, &ln2cnt_map,
					       layout_name);
...
gde2kbd_status_prepare_drawing (Gde2kbdStatus * gki, int group)
{
	...
	if (globals.ind_cfg.show_flags) {
		...
		guchar *pixbuf_data;
		gde2kbd_status_render_cairo (cairo_create (cs), group);
...
static void
gde2kbd_status_update_tooltips (Gde2kbdStatus * gki)
{
	...
	gde2kbd_status_set_tooltips (gki, buf);
	g_free (buf);
}

void
gde2kbd_status_reinit_ui (Gde2kbdStatus * gki)
{
	gde2kbd_status_global_cleanup (gki);
	gde2kbd_status_global_fill (gki);

	gde2kbd_status_set_current_page (gki);
}

/* Should be called once for all widgets */
static void
gde2kbd_status_cfg_changed (GSettings *settings,
			    gchar     *key,
			    gpointer   user_data)
{
	xkl_debug (100,
	...
	ForAllIndicators () {
		gde2kbd_status_reinit_ui (gki);
	} NextIndicator ();
}

/* Should be called once for all widgets */
static void
gde2kbd_status_ind_cfg_changed (GSettings *settings,
				gchar     *key,
				gpointer   user_data)
{
	xkl_debug (100,
...
static void
gde2kbd_status_load_group_names (const gchar ** layout_ids,
...
/* Should be called once for all widgets */
static void
gde2kbd_status_kbd_cfg_callback (Gde2kbdStatus * gki)
{
	...
	gde2kbd_status_load_group_names ((const gchar **) xklrec->layouts,
...
static void
gde2kbd_status_state_callback (XklEngine * engine,
			    ...
			    gint group, gboolean restore)
{
	...
	if (changeType == GROUP_CHANGED) {
		ForAllIndicators () {
			...
			gde2kbd_status_set_current_page_for_group (gki,
								group);
		}
		NextIndicator ();
	}
...
gde2kbd_status_set_current_page_for_group (Gde2kbdStatus * gki, int group)
{
	...
	gde2kbd_status_update_tooltips (gki);
}

/* Should be called once for all widgets */
static GdkFilterReturn
gde2kbd_status_filter_x_evt (GdkXEvent * xev, GdkEvent * event)
{
...
/* Should be called once for all widgets */
static void
gde2kbd_status_start_listen (void)
{
	gdk_window_add_filter (NULL, (GdkFilterFunc)
			       gde2kbd_status_filter_x_evt, NULL);
	gdk_window_add_filter (gdk_get_default_root_window (),
			       (GdkFilterFunc) gde2kbd_status_filter_x_evt,
			       NULL);
...
/* Should be called once for all widgets */
static void
gde2kbd_status_stop_listen (void)
{
	...
	gdk_window_remove_filter
	    (gdk_get_default_root_window (),
	     (GdkFilterFunc) gde2kbd_status_filter_x_evt, NULL);
}

static void
gde2kbd_status_size_changed (Gde2kbdStatus * gki, gint size)
{
...
static void
gde2kbd_status_theme_changed (GtkSettings * settings, GParamSpec * pspec,
			   Gde2kbdStatus * gki)
{
	gde2kbd_indicator_config_refresh_style (&globals.ind_cfg);
	gde2kbd_status_reinit_ui (gki);
}

static void
gde2kbd_status_init (Gde2kbdStatus * gki)
{
	int i;

	if (!g_slist_length (globals.widget_instances))
		gde2kbd_status_global_init ();

	gki->priv = g_new0 (Gde2kbdStatusPrivate, 1);
	...
	if (globals.engine == NULL) {
		gde2kbd_status_set_tooltips (gki,
					  _("XKB initialization error"));
		return;
	}

	gde2kbd_status_set_tooltips (gki, NULL);
	...
	g_signal_connect (gki, "size-changed",
			  G_CALLBACK (gde2kbd_status_size_changed), NULL);
	g_signal_connect (gki, "activate",
			  G_CALLBACK (gde2kbd_status_activate), NULL);
	...
	for (i = sizeof (settings_signal_names) /
		...
		gki->priv->settings_signal_handlers[i] =
		    g_signal_connect_after (gtk_settings_get_default (),
					    ...
					    G_CALLBACK
					    (gde2kbd_status_theme_changed),
					    gki);
}

static void
gde2kbd_status_finalize (GObject * obj)
{
	int i;
	Gde2kbdStatus *gki = GDE2KBD_STATUS (obj);
	xkl_debug (100,
		   "Starting the gde2-kbd-status widget shutdown process for %p\n",
		   gki);
	...
	xkl_debug (100,
		   "The instance of gde2-kbd-status successfully finalized\n");

	g_free (gki->priv);

	G_OBJECT_CLASS (gde2kbd_status_parent_class)->finalize (obj);

	if (!g_slist_length (globals.widget_instances))
		gde2kbd_status_global_term ();
}

static void
gde2kbd_status_global_term (void)
{
	xkl_debug (100, "*** Last  Gde2kbdStatus instance *** \n");
	gde2kbd_status_stop_listen ();
...
static void
gde2kbd_status_class_init (Gde2kbdStatusClass * klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	xkl_debug (100, "*** First Gde2kbdStatus instance *** \n");
	...
	/* Initing vtable */
	object_class->finalize = gde2kbd_status_finalize;
...
gde2kbd_status_global_init (void)
{
	...
	globals.state_changed_handler =
	    g_signal_connect (globals.engine, "X-state-changed",
			      G_CALLBACK (gde2kbd_status_state_callback),
			      NULL);
	globals.config_changed_handler =
	    g_signal_connect (globals.engine, "X-config-changed",
			      G_CALLBACK (gde2kbd_status_kbd_cfg_callback),
			      NULL);
	...
	gde2kbd_desktop_config_start_listen (&globals.cfg,
					  (GCallback)
					  gde2kbd_status_cfg_changed, NULL);
	gde2kbd_indicator_config_start_listen (&globals.ind_cfg,
					    (GCallback)
					    gde2kbd_status_ind_cfg_changed,
					    NULL);
	...
	gde2kbd_status_start_listen ();

	xkl_debug (100, "*** Inited globals *** \n");
}

GtkStatusIcon *
gde2kbd_status_new (void)
{
	return
	    GTK_STATUS_ICON (g_object_new (gde2kbd_status_get_type (), NULL));
}

/**
 * gde2kbd_status_get_xkl_engine:
 *
 * Returns: (transfer none): The engine shared by all Gde2kbdStatus objects
 */
XklEngine *
gde2kbd_status_get_xkl_engine ()
{
...
/**
 * gde2kbd_status_get_group_names:
 *
...
gchar *
gde2kbd_status_get_image_filename (guint group)
{
	if (!globals.ind_cfg.show_flags)
		return NULL;
...
void
gde2kbd_status_set_angle (Gde2kbdStatus * gki, gdouble angle)
{
