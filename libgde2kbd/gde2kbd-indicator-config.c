...
/**
 * Gde2kbdIndicatorConfig:
 */
#define GDE2KBD_INDICATOR_CONFIG_SCHEMA  GDE2KBD_CONFIG_SCHEMA ".indicator"

const gchar GDE2KBD_INDICATOR_CONFIG_KEY_SHOW_FLAGS[] = "show-flags";
const gchar GDE2KBD_INDICATOR_CONFIG_KEY_SECONDARIES[] = "secondary";
const gchar GDE2KBD_INDICATOR_CONFIG_KEY_FONT_FAMILY[] = "font-family";
const gchar GDE2KBD_INDICATOR_CONFIG_KEY_FOREGROUND_COLOR[] = "foreground-color";
const gchar GDE2KBD_INDICATOR_CONFIG_KEY_BACKGROUND_COLOR[] = "background-color";

#define SYSTEM_FONT_SCHEMA "org.gde2.interface"
 ...
 */
static void
gde2kbd_indicator_config_load_font (Gde2kbdIndicatorConfig * ind_config)
{
	ind_config->font_family =
	    g_settings_get_string (ind_config->settings,
				   GDE2KBD_INDICATOR_CONFIG_KEY_FONT_FAMILY);
...
static void
gde2kbd_indicator_config_load_colors (Gde2kbdIndicatorConfig * ind_config)
{
	ind_config->foreground_color =
	    g_settings_get_string (ind_config->settings,
	                           GDE2KBD_INDICATOR_CONFIG_KEY_FOREGROUND_COLOR);
	...
	ind_config->background_color =
	    g_settings_get_string (ind_config->settings,
				     GDE2KBD_INDICATOR_CONFIG_KEY_BACKGROUND_COLOR);
}

void
gde2kbd_indicator_config_refresh_style (Gde2kbdIndicatorConfig * ind_config)
{
	g_free (ind_config->font_family);
	g_free (ind_config->foreground_color);
	g_free (ind_config->background_color);
	gde2kbd_indicator_config_load_font (ind_config);
	gde2kbd_indicator_config_load_colors (ind_config);
}
				       ...
				       kbd_config, int group)
{
	...
	if ((kbd_config->layouts_variants != NULL) &&
		...
		if (full_layout_name != NULL) {
			char *l, *v;
			gde2kbd_keyboard_config_split_items (full_layout_name,
							  &l, &v);
			if (l != NULL) {
...
void
gde2kbd_indicator_config_load_image_filenames (Gde2kbdIndicatorConfig *
					    ind_config,
					    Gde2kbdKeyboardConfig *
					    kbd_config)
{
	...
	for (i = xkl_engine_get_max_num_groups (ind_config->engine);
	     --i >= 0;) {
		char *image_file =
		    gde2kbd_indicator_config_get_images_file (ind_config,
							   kbd_config,
							   i);
...
void
gde2kbd_indicator_config_free_image_filenames (Gde2kbdIndicatorConfig *
					    ind_config)
{
...
void
gde2kbd_indicator_config_init (Gde2kbdIndicatorConfig * ind_config,
			       XklEngine * engine)
{
	gchar *sp;

	memset (ind_config, 0, sizeof (*ind_config));
	ind_config->settings = g_settings_new (GDE2KBD_INDICATOR_CONFIG_SCHEMA);
...
void
gde2kbd_indicator_config_term (Gde2kbdIndicatorConfig * ind_config)
{
	g_free (ind_config->font_family);
	...
	gde2kbd_indicator_config_free_image_filenames (ind_config);
...
void
gde2kbd_indicator_config_load_from_gsettings (Gde2kbdIndicatorConfig * ind_config)
{
	ind_config->secondary_groups_mask =
	    g_settings_get_int (ind_config->settings,
				GDE2KBD_INDICATOR_CONFIG_KEY_SECONDARIES);

	ind_config->show_flags =
	    g_settings_get_boolean (ind_config->settings,
				 GDE2KBD_INDICATOR_CONFIG_KEY_SHOW_FLAGS);
...
void
gde2kbd_indicator_config_save_to_gsettings (Gde2kbdIndicatorConfig * ind_config)
{
...
void
gde2kbd_indicator_config_activate (Gde2kbdIndicatorConfig * ind_config)
{
...
/**
 * gde2kbd_indicator_config_start_listen:
 * @func: (scope notified): a function to call when settings are changed
 */
void
gde2kbd_indicator_config_start_listen (Gde2kbdIndicatorConfig *
				    ind_config,
				    GCallback func,
				    gpointer user_data)
{
...
void
gde2kbd_indicator_config_stop_listen (Gde2kbdIndicatorConfig * ind_config)
{
