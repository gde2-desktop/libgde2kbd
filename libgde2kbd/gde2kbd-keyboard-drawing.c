...
#include <gde2kbd-keyboard-drawing.h>
#include <gde2kbd-keyboard-drawing-marshal.h>
#include <gde2kbd-util.h>
...
static guint gde2kbd_keyboard_drawing_signals[NUM_SIGNALS] = { 0 };

static void gde2kbd_keyboard_drawing_set_mods (Gde2kbdKeyboardDrawing * drawing,
					    guint mods);
...
static gint
xkb_to_pixmap_coord (Gde2kbdKeyboardDrawingRenderContext * context, gint n)
{
	return n * context->scale_numerator / context->scale_denominator;
}

static gdouble
xkb_to_pixmap_double (Gde2kbdKeyboardDrawingRenderContext * context,
		      gdouble d)
{
...
static void
draw_polygon (Gde2kbdKeyboardDrawingRenderContext * context,
...
static void
draw_rectangle (Gde2kbdKeyboardDrawingRenderContext * context,
#if GTK_CHECK_VERSION (3, 0, 0)
		GdkRGBA * fill_color,
#else
		GdkColor * fill_color,
...
static void
draw_outline (Gde2kbdKeyboardDrawingRenderContext * context,
...
static guint
find_keycode (Gde2kbdKeyboardDrawing * drawing, gchar * key_name)
{
...
static void
set_markup (Gde2kbdKeyboardDrawingRenderContext * context, gchar *txt)
{
...
static void
set_key_label_in_layout (Gde2kbdKeyboardDrawingRenderContext * context,
			 guint keyval)
{
	gchar buf[5];
	gunichar uc;
...
static void
draw_pango_layout (Gde2kbdKeyboardDrawingRenderContext * context,
		   Gde2kbdKeyboardDrawing * drawing,
...
static void
draw_key_label_helper (Gde2kbdKeyboardDrawingRenderContext * context,
		       Gde2kbdKeyboardDrawing * drawing,
		       KeySym keysym,
		       gint angle,
		       Gde2kbdKeyboardDrawingGroupLevelPosition glp,
		       gint x,
		       gint y, gint width, gint height, gint padding)
{
	...
	switch (glp) {
	case GDE2KBD_KEYBOARD_DRAWING_POS_TOPLEFT:
	case GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT:
		{
			ycell =
			    glp == GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT;
	...
	case GDE2KBD_KEYBOARD_DRAWING_POS_TOPRIGHT:
	case GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT:
		{
			ycell =
			    glp == GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT;
...
static void
draw_key_label (Gde2kbdKeyboardDrawingRenderContext * context,
		Gde2kbdKeyboardDrawing * drawing,
		guint keycode,
		gint angle,
		gint xkb_origin_x,
		gint xkb_origin_y, gint xkb_width, gint xkb_height)
{
	...
	for (glp = GDE2KBD_KEYBOARD_DRAWING_POS_TOPLEFT;
	     glp < GDE2KBD_KEYBOARD_DRAWING_POS_TOTAL; glp++) {
...
/* groups are from 0-3 */
static void
draw_key (Gde2kbdKeyboardDrawingRenderContext * context,
	  Gde2kbdKeyboardDrawing * drawing, Gde2kbdKeyboardDrawingKey * key)
{
	XkbShapeRec *shape;
#if GTK_CHECK_VERSION (3, 0, 0)
...
static void
invalidate_region (Gde2kbdKeyboardDrawing * drawing,
		   gdouble angle,
...
static void
invalidate_indicator_doodad_region (Gde2kbdKeyboardDrawing * drawing,
				    Gde2kbdKeyboardDrawingDoodad * doodad)
{
	if (!drawing->xkb)
		return;
...
static void
invalidate_key_region (Gde2kbdKeyboardDrawing * drawing,
		       Gde2kbdKeyboardDrawingKey * key)
{
	if (!drawing->xkb)
		return;
...
static void
draw_text_doodad (Gde2kbdKeyboardDrawingRenderContext * context,
		  Gde2kbdKeyboardDrawing * drawing,
		  Gde2kbdKeyboardDrawingDoodad * doodad,
...
static void
draw_indicator_doodad (Gde2kbdKeyboardDrawingRenderContext * context,
		       Gde2kbdKeyboardDrawing * drawing,
		       Gde2kbdKeyboardDrawingDoodad * doodad,
...
static void
draw_shape_doodad (Gde2kbdKeyboardDrawingRenderContext * context,
		   Gde2kbdKeyboardDrawing * drawing,
		   Gde2kbdKeyboardDrawingDoodad * doodad,
...
static void
draw_doodad (Gde2kbdKeyboardDrawingRenderContext * context,
	     Gde2kbdKeyboardDrawing * drawing,
	     Gde2kbdKeyboardDrawingDoodad * doodad)
{
...
typedef struct {
	Gde2kbdKeyboardDrawing *drawing;
	Gde2kbdKeyboardDrawingRenderContext *context;
} DrawKeyboardItemData;

static void
redraw_overlapping_doodads (Gde2kbdKeyboardDrawingRenderContext * context,
			    Gde2kbdKeyboardDrawing * drawing,
			    Gde2kbdKeyboardDrawingKey * key)
{
	GList *list;
	...
	for (list = drawing->keyboard_items; list; list = list->next) {
		Gde2kbdKeyboardDrawingItem *item = list->data;

		if (do_draw
		    && item->type ==
		    GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_DOODAD)
			draw_doodad (context, drawing,
				     (Gde2kbdKeyboardDrawingDoodad *) item);
...
static void
draw_keyboard_item (Gde2kbdKeyboardDrawingItem * item,
		    DrawKeyboardItemData * data)
{
	Gde2kbdKeyboardDrawing *drawing = data->drawing;
	Gde2kbdKeyboardDrawingRenderContext *context = data->context;

	if (!drawing->xkb)
		return;

	switch (item->type) {
	case GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_INVALID:
		break;

	case GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY:
	case GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY_EXTRA:
		draw_key (context, drawing,
			  (Gde2kbdKeyboardDrawingKey *) item);
		break;

	case GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_DOODAD:
		draw_doodad (context, drawing,
			     (Gde2kbdKeyboardDrawingDoodad *) item);
		break;
	}
...
static void
draw_keyboard_to_context (Gde2kbdKeyboardDrawingRenderContext * context,
			  Gde2kbdKeyboardDrawing * drawing)
{
...
static gboolean
create_cairo (Gde2kbdKeyboardDrawing * drawing)
{
#if GTK_CHECK_VERSION (3, 0, 0)
...
static void
destroy_cairo (Gde2kbdKeyboardDrawing * drawing)
{
...
static void
draw_keyboard (Gde2kbdKeyboardDrawing * drawing)
{
#if GTK_CHECK_VERSION (3, 0, 0)
...
static void
alloc_render_context (Gde2kbdKeyboardDrawing * drawing)
{
	Gde2kbdKeyboardDrawingRenderContext *context =
	    drawing->renderContext =
	    g_new0 (Gde2kbdKeyboardDrawingRenderContext, 1);
...
static void
free_render_context (Gde2kbdKeyboardDrawing * drawing)
{
	Gde2kbdKeyboardDrawingRenderContext *context = drawing->renderContext;
...
{
	Gde2kbdKeyboardDrawing *drawing = user_data;
...
static gboolean
context_setup_scaling (Gde2kbdKeyboardDrawingRenderContext * context,
		       Gde2kbdKeyboardDrawing * drawing,
...
size_allocate (GtkWidget * widget,
	       GtkAllocation * allocation, Gde2kbdKeyboardDrawing * drawing)
{
	Gde2kbdKeyboardDrawingRenderContext *context = drawing->renderContext;
...
key_event (GtkWidget * widget,
	   GdkEventKey * event, Gde2kbdKeyboardDrawing * drawing)
{
	Gde2kbdKeyboardDrawingKey *key;
	if (!drawing->xkb)
		return FALSE;
	...
	if (event->hardware_keycode > drawing->xkb->max_key_code ||
		...
		g_signal_emit (drawing,
			       gde2kbd_keyboard_drawing_signals[BAD_KEYCODE],
...
button_press_event (GtkWidget * widget,
		    GdkEventButton * event, Gde2kbdKeyboardDrawing * drawing)
{
	if (!drawing->xkb)
		return FALSE;
...
static gboolean
unpress_keys (Gde2kbdKeyboardDrawing * drawing)
{
	gint i;
...
focus_event (GtkWidget * widget,
	     GdkEventFocus * event, Gde2kbdKeyboardDrawing * drawing)
{
...
static gint
compare_keyboard_item_priorities (Gde2kbdKeyboardDrawingItem * a,
				  Gde2kbdKeyboardDrawingItem * b)
{
...
static void
init_indicator_doodad (Gde2kbdKeyboardDrawing * drawing,
...
static void
init_keys_and_doodads (Gde2kbdKeyboardDrawing * drawing)
{
	gint i, j, k;
	gint x, y;
	...
	for (i = 0; i < drawing->xkb->geom->num_doodads; i++) {
		...
		Gde2kbdKeyboardDrawingDoodad *doodad =
		    g_new (Gde2kbdKeyboardDrawingDoodad, 1);

		doodad->type = GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_DOODAD;
		doodad->origin_x = 0;
		doodad->origin_y = 0;
		doodad->angle = 0;
		...
		for (j = 0; j < section->num_rows; j++) {
			...
			for (k = 0; k < row->num_keys; k++) {
				...
				if (keycode >= drawing->xkb->min_key_code
					...
					if (key->type ==
					    GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_INVALID)
					{
						key->type =
						    GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY;
					} else {
						/* duplicate key for the same keycode,
						   already defined as GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY */
						key =
						    g_new0
						    (Gde2kbdKeyboardDrawingKey,
						     1);
						key->type =
					...
					key =
					    g_new0 (Gde2kbdKeyboardDrawingKey,
						    1);
					key->type =
					    GDE2KBD_KEYBOARD_DRAWING_ITEM_TYPE_KEY_EXTRA;
				}
...
static void
init_colors (Gde2kbdKeyboardDrawing * drawing)
{
	gboolean result;
	gint i;
		  ...
		  Gde2kbdKeyboardDrawing * drawing)
{
	...
	for (itemp = drawing->keyboard_items; itemp; itemp = itemp->next) {
		Gde2kbdKeyboardDrawingItem *item = itemp->data;
	...
	if (((XEvent *) gdkxev)->type == drawing->xkb_event_type) {
		...
		case XkbStateNotify:
			if (((kev->state.changed & modifier_change_mask) &&
				...
				free_cdik (drawing);
				if (drawing->track_modifiers)
					gde2kbd_keyboard_drawing_set_mods
					    (drawing,
...
static void
destroy (Gde2kbdKeyboardDrawing * drawing)
{
...
static void
style_changed (Gde2kbdKeyboardDrawing * drawing)
{
	pango_layout_context_changed (drawing->renderContext->layout);
}

static void
gde2kbd_keyboard_drawing_init (Gde2kbdKeyboardDrawing * drawing)
{
...
GtkWidget *
gde2kbd_keyboard_drawing_new (void)
{
	return
	    GTK_WIDGET (g_object_new
			(gde2kbd_keyboard_drawing_get_type (), NULL));
}

static void
gde2kbd_keyboard_drawing_class_init (Gde2kbdKeyboardDrawingClass * klass)
{
	klass->bad_keycode = NULL;

	gde2kbd_keyboard_drawing_signals[BAD_KEYCODE] =
	    g_signal_new ("bad-keycode", gde2kbd_keyboard_drawing_get_type (),
			  G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (Gde2kbdKeyboardDrawingClass,
					   bad_keycode), NULL, NULL,
			  gde2kbd_keyboard_drawing_VOID__UINT, G_TYPE_NONE, 1,
			  G_TYPE_UINT);
}

GType
gde2kbd_keyboard_drawing_get_type (void)
{
	static GType gde2kbd_keyboard_drawing_type = 0;

	if (!gde2kbd_keyboard_drawing_type) {
		static const GTypeInfo gde2kbd_keyboard_drawing_info = {
			sizeof (Gde2kbdKeyboardDrawingClass),
			NULL,	/* base_init */
			NULL,	/* base_finalize */
			(GClassInitFunc) gde2kbd_keyboard_drawing_class_init,
			NULL,	/* class_finalize */
			NULL,	/* class_data */
			sizeof (Gde2kbdKeyboardDrawing),
			0,	/* n_preallocs */
			(GInstanceInitFunc) gde2kbd_keyboard_drawing_init,
		};

		gde2kbd_keyboard_drawing_type =
		    g_type_register_static (GTK_TYPE_DRAWING_AREA,
					    "Gde2kbdKeyboardDrawing",
					    &gde2kbd_keyboard_drawing_info,
					    0);
	}

	return gde2kbd_keyboard_drawing_type;
}

void
gde2kbd_keyboard_drawing_set_mods (Gde2kbdKeyboardDrawing * drawing, guint mods)
{
#ifdef KBDRAW_DEBUG
	printf ("set_mods: %d\n", mods);
#endif
...
/**
 * gde2kbd_keyboard_drawing_render:
 ...
 */
gboolean
gde2kbd_keyboard_drawing_render (Gde2kbdKeyboardDrawing * kbdrawing,
			      cairo_t * cr,
			      PangoLayout * layout,
			      double x, double y,
...
gboolean
gde2kbd_keyboard_drawing_set_keyboard (Gde2kbdKeyboardDrawing * drawing,
...
const gchar* gde2kbd_keyboard_drawing_get_keycodes(Gde2kbdKeyboardDrawing* drawing)
{
...
const gchar* gde2kbd_keyboard_drawing_get_geometry(Gde2kbdKeyboardDrawing* drawing)
{
...
const gchar* gde2kbd_keyboard_drawing_get_symbols(Gde2kbdKeyboardDrawing* drawing)
{
...
const gchar* gde2kbd_keyboard_drawing_get_types(Gde2kbdKeyboardDrawing* drawing)
{
...
const gchar* gde2kbd_keyboard_drawing_get_compat(Gde2kbdKeyboardDrawing* drawing)
{
...
void
gde2kbd_keyboard_drawing_set_track_modifiers (Gde2kbdKeyboardDrawing * drawing,
					   gboolean enable)
{
	if (enable) {
		XkbStateRec state;
		...
		gde2kbd_keyboard_drawing_set_mods (drawing,
						state.compat_state);
	} else
		drawing->track_modifiers = 0;
}
...
void
gde2kbd_keyboard_drawing_set_track_config (Gde2kbdKeyboardDrawing * drawing,
					gboolean enable)
{
	if (enable)
...
void
gde2kbd_keyboard_drawing_set_groups_levels (Gde2kbdKeyboardDrawing * drawing,
					 Gde2kbdKeyboardDrawingGroupLevel *
					 groupLevels[])
{
#ifdef KBDRAW_DEBUG
	printf ("set_group_levels [topLeft]: %d %d \n",
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_TOPLEFT]->group,
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_TOPLEFT]->level);
	printf ("set_group_levels [topRight]: %d %d \n",
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_TOPRIGHT]->group,
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_TOPRIGHT]->level);
	printf ("set_group_levels [bottomLeft]: %d %d \n",
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT]->group,
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMLEFT]->level);
	printf ("set_group_levels [bottomRight]: %d %d \n",
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT]->group,
		groupLevels[GDE2KBD_KEYBOARD_DRAWING_POS_BOTTOMRIGHT]->level);
#endif
...
static void
gde2kbd_keyboard_drawing_begin_print (GtkPrintOperation * operation,
...
static void
gde2kbd_keyboard_drawing_draw_page (GtkPrintOperation * operation,
				 GtkPrintContext * context,
				 gint page_nr,
				 XkbLayoutPreviewPrintData * data)
{
	...
	gde2kbd_keyboard_drawing_render (GDE2KBD_KEYBOARD_DRAWING
...
void
gde2kbd_keyboard_drawing_print (Gde2kbdKeyboardDrawing * drawing,
			     ...
			     const gchar * description)
{
	...
	g_signal_connect (print, "begin_print",
			  G_CALLBACK (gde2kbd_keyboard_drawing_begin_print),
			  &data);
	g_signal_connect (print, "draw_page",
			  G_CALLBACK (gde2kbd_keyboard_drawing_draw_page),
			  &data);
...
GtkWidget *
gde2kbd_keyboard_drawing_new_dialog (gint group, gchar * group_name)
{
	...
	kbdraw = gde2kbd_keyboard_drawing_new ();
	...
	if (xkl_config_rec_get_from_server (xkl_data, engine)) {
		...
		{
			gde2kbd_keyboard_drawing_set_keyboard
			    (GDE2KBD_KEYBOARD_DRAWING (kbdraw),
			     &component_names);
	...
	rect = gde2kbd_preview_load_position ();
