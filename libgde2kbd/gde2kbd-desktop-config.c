...
/**
 * Gde2kbdDesktopConfig:
 */
#define GDE2KBD_DESKTOP_CONFIG_SCHEMA  GDE2KBD_CONFIG_SCHEMA ".general"

const gchar GDE2KBD_DESKTOP_CONFIG_KEY_DEFAULT_GROUP[] = "default-group";
const gchar GDE2KBD_DESKTOP_CONFIG_KEY_GROUP_PER_WINDOW[] = "group-per-window";
const gchar GDE2KBD_DESKTOP_CONFIG_KEY_HANDLE_INDICATORS[] = "handle-indicators";
const gchar GDE2KBD_DESKTOP_CONFIG_KEY_LAYOUT_NAMES_AS_GROUP_NAMES[] = "layout-names-as-group-names";
const gchar GDE2KBD_DESKTOP_CONFIG_KEY_LOAD_EXTRA_ITEMS[] = "load-extra-items";
...
static gboolean
    gde2kbd_desktop_config_get_lv_descriptions
    (Gde2kbdDesktopConfig * config,
...
/*
 * extern Gde2kbdDesktopConfig config functions
 */
void
gde2kbd_desktop_config_init (Gde2kbdDesktopConfig * config,
			  XklEngine * engine)
{
	memset (config, 0, sizeof (*config));
	config->settings = g_settings_new (GDE2KBD_DESKTOP_CONFIG_SCHEMA);
	config->engine = engine;
}

void
gde2kbd_desktop_config_term (Gde2kbdDesktopConfig * config)
{
...
void
gde2kbd_desktop_config_save_to_gsettings (Gde2kbdDesktopConfig * config)
{
...
gboolean
gde2kbd_desktop_config_activate (Gde2kbdDesktopConfig * config)
{
	gboolean rv = TRUE;
...
void
gde2kbd_desktop_config_lock_next_group (Gde2kbdDesktopConfig * config)
{
...
void
gde2kbd_desktop_config_lock_prev_group (Gde2kbdDesktopConfig * config)
{
...
void
gde2kbd_desktop_config_restore_group (Gde2kbdDesktopConfig * config)
{
...
/**
 * gde2kbd_desktop_config_start_listen:
 * @func: (scope notified): a function to call when settings are changed
 */
void
gde2kbd_desktop_config_start_listen (Gde2kbdDesktopConfig * config,
				  GCallback func,
				  gpointer user_data)
{
...
void
gde2kbd_desktop_config_stop_listen (Gde2kbdDesktopConfig * config)
{
					     ...
					     gchar *** full_group_names)
{
	...
	if (!gde2kbd_desktop_config_get_lv_descriptions
	...
	*full_group_names = pfgn =
	...
	while (plld != NULL && *plld != NULL) {
		...
		psld++, psvd++;
		*psgn++ = g_strdup (sd);
		*pfgn++ = g_strdup (gde2kbd_keyboard_config_format_full_layout
				    (*plld++, *plvd++));
	}
	g_strfreev (sld);
	g_strfreev (lld);
